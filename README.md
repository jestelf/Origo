...
📂 Основные каталоги
- `assets/` — игровые и редакторские ресурсы
- `configs/` — YAML конфигурации движка
- `tests/` — автотесты и бенчмарки
...
- **`scripts/`** — Автоматизация и утилиты  
  - CLI-интерфейс: `cli.py` (команды `run`, `assets`, `build`)
...
- **`tests/`** — автоматические тесты и бенчмарки
...
python main.py          # запуск редактора или движка
````
...
Дополнительные задачи можно выполнять через CLI-интерфейс:

```bash
python scripts/cli.py --help
```
* `run <scene>` — запуск указанной сцены
* `assets` — вывод списка файлов в `assets/`
* `build --platform <p>` — сборка проекта
...
* CLI-плагины: `origo_cli/plugins/` (пути можно указать в `ORIGO_CLI_PLUGIN_PATHS`, каждый плагин реализует `register(cli)`)
* Внешние расширения: `extensions/` (или каталоги из `ORIGO_EXT_PATHS`; подключаются функцией `load_extensions(engine)`)
